...
3. Fusionne avec donnees_communes.csv (incluant PTOT, PMUN, LIBGEO, etc.)
...
8. Filtre les communes PMUN ∈ [10000, 20000]
9. Sauvegarde sous `data/donnees.csv`
"""
import os
import sys
import pandas as pd
...
def main():
    ...
    comm['PMUN'] = pd.to_numeric(comm['PMUN'], errors='coerce').fillna(0).astype(int)
    ...
    for fn in sorted(os.listdir(equip_dir)):
        ...
        if not nb_cols:
            continue
        df[nb_cols] = df[nb_cols].apply(pd.to_numeric, errors='coerce').fillna(0).astype(int)
    ...
    for d, lbl in labels.items():
        raw_col = f"{d}_{lbl}"
        ratio_col = f"{d}_{lbl}_1000"
        final[ratio_col] = pd.to_numeric(
            final[raw_col] / (final['PTOT'] / 1000),
            errors='coerce'
        ).fillna(0).round(2)

    # 8. Filtre PMUN entre 10000 et 20000
    final = final[(final['PMUN'] >= 10000) & (final['PMUN'] <= 20000)].copy()
    ...
    try:
        ...
        print(f"✅ Base filtrée générée : {out_f}")
